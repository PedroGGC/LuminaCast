...
async def sync_media_db(request: Request, db: Session = Depends(get_db)):
    tmdb_key = os.getenv("TMDB_API_KEY", "").strip()
    if not tmdb_key:
        raise HTTPException(status_code=500, detail="TMDB_API_KEY não configurada.")

    # Limpa banco opcionalmente ou mantém (para evitar duplicatas)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for title in CARTOON_TITLES:
            ...
            search_resp = await client.get(
                f"{TMDB_BASE}/search/tv", params={**params_base, "query": title}
            )
